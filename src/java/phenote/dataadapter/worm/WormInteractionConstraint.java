...
public class WormInteractionConstraint extends AbstractCommitConstraint {
  ...
  protected ConstraintStatus checkCharCommit(CharacterI chr) {
    ...
    try {
      ...
      if ( (pgdbid == null) || (pgdbid == "") ) { 
        String intID = chr.getValueString("Interaction ID");
        message += "New character for "+intID+".  ";  warning = true;	// give warning for any new characters
        pgdbid = "with new value for "+intID;
      }
...
//            warning = true; message += "Character "+pgdbid+" Mapped Wrong and has no Object Remark.  "; } } }
      ...
      if ( (chr.hasValue("Effector")) || (chr.hasValue("tor Variation")) || (chr.hasValue("tor Transgene")) ) { } else { 
        flagBad++;
        message += "Character "+pgdbid+" has neither Effector nor tor Variation nor tor Transgene.  "; }

      if (chr.hasValue("Type")) { } else { 
        flagBad++;
        message += "Character "+pgdbid+" has no Type.  "; }

      if ( (chr.hasValue("Effected")) || (chr.hasValue("ted Variation")) || (chr.hasValue("ted Transgene")) ) { } else { 
        flagBad++;
