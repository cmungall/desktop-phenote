...
//    log().debug("Setting SessionManagers session");
    ...
    //    log().debug("SessionManager's session is set");
    oboSession = s;
  }
  ...
  public CharField getDateCreatedField() {
    ...
    try { 
        CharField dateField = getCharFieldForEnum(dateEnum);
        dateField.setType(Type.DATE);  // Shouldn't have to do this, but it doesn't seem to get set like it should.
        // should dateEnum have autogen state?
        dateField.setIsAutoGenerated(true);
        //        log().debug("dateEnum = " + dateEnum + ", dateField = " + dateField); // DEL
        //        log().debug("getDateCreatedField: got dateField, type = " + dateField.type + "; set IsAutoGenerated to true: " + dateField); // DEL
        return dateField;
    }
    // Doesnt exist yet - Add to datamodel and config(?)
      ...
      // should dateEnum have autogen state?
      dateField.setIsAutoGenerated(true);
      //      log().debug("getDateCreatedField: didn't find dateField--had to make new one. Set IsAutoGenerated to true: " + dateField); // DEL
      addField(dateField);
